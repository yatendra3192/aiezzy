...
export default function PlanError({ error, reset }: { error: Error; reset: () => void }) {
  useEffect(() => {
    reportError(error, { type: 'error-boundary', page: 'plan' });
  }, [error]);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="bg-bg-surface border border-border-subtle rounded-2xl p-8 max-w-md text-center space-y-4">
        ...
        <h2 className="font-display text-lg font-bold text-text-primary">Plan page error</h2>
        <p className="text-text-muted text-sm font-body">There was a problem loading the trip planner.</p>
