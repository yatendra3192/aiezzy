...
export default function SettingsError({ error, reset }: { error: Error; reset: () => void }) {
  useEffect(() => {
    reportError(error, { type: 'error-boundary', page: 'settings' });
  }, [error]);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="bg-bg-surface border border-border-subtle rounded-2xl p-8 max-w-md text-center space-y-4">
        ...
        <h2 className="font-display text-lg font-bold text-text-primary">Settings error</h2>
        <p className="text-text-muted text-sm font-body">There was a problem loading your settings.</p>
        <div className="flex items-center gap-3 justify-center">
          <a href="/my-trips" className="border border-border-subtle text-text-secondary font-display font-bold text-sm py-2.5 px-6 rounded-xl hover:bg-bg-card transition-colors">
            My Trips
          </a>
