...
export default function DeepPlanError({ error, reset }: { error: Error; reset: () => void }) {
  useEffect(() => {
    reportError(error, { type: 'error-boundary', page: 'deep-plan' });
  }, [error]);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="bg-bg-surface border border-border-subtle rounded-2xl p-8 max-w-md text-center space-y-4">
        ...
        <h2 className="font-display text-lg font-bold text-text-primary">Itinerary page error</h2>
        <p className="text-text-muted text-sm font-body">There was a problem generating your day-by-day plan. Your trip and selections are safe.</p>
        <div className="flex items-center gap-3 justify-center">
          <a href="/route" className="border border-border-subtle text-text-secondary font-display font-bold text-sm py-2.5 px-6 rounded-xl hover:bg-bg-card transition-colors">
            Back to Route
          </a>
