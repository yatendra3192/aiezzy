...
- `/api/ai/itinerary-activities` — **Gemini 3.1 Flash Lite** (primary), OpenAI Chat Completions (fallback), Anthropic (last resort). Generates city activities with durations, categories, opening hours, ticket prices, and **GPS coordinates** (`lat`/`lng`). Per-provider 30s timeout with 3 retries for OpenAI. Multi-day aware: assigns `dayIndex` per activity + `dayThemes` for logical day progression. No static fallback — returns empty array if all providers fail; UI shows "Auto-plan" prompt. Response cached in `deepPlanData.cityActivities`. Requests 7 activities per day + 3 extras. Auto-fill throttled to **2 concurrent** city fetches. **Critical:** OpenAI Responses API (`/v1/responses`) is unreachable from Railway asia-southeast1 — use Gemini or Chat Completions only. **All AI routes require authentication** (`getServerSession`).
- `/api/restaurants` — Google Places Nearby Search for meal venue suggestions. Takes `lat`, `lng`, `mealType` (or `city` for geocoding fallback). Returns 5 restaurants with name, rating, cuisine, coordinates. 30-minute server cache. Used by deep plan meal slots.
...
**Inter-activity travel times:** Real walk/transit/drive times fetched via Google Directions between consecutive stops on all day types. **GPS-based routing:** When activities have `lat`/`lng` (from AI), directions queries use `"lat,lng"` coordinates instead of vague `"Activity Name, City"` strings — gives precise point-to-point times. Falls back to name+city when no coordinates. Hotel stops ("Leave hotel", "Return to hotel") also carry hotel lat/lng for accurate first/last-mile times. `parseDistKm()` helper handles km/mi/ft/m (Google returns different units by locale). Distances < 100m are treated as bogus (directions resolution failed) and skipped in cost calculations. Dropdown (z-50) lets user switch modes; "Directions" link opens Google Maps with selected mode. On travel/departure days, changing mode recalculates "Leave by X to reach on time" note. **City context for directions:** stops before the transport leg use `departureCity`, stops after use `day.city` (destination). The cascade code accounts for activity durations: `departureMin = stopStartTime + durationMin` for attractions (not just `stopStartTime`). When activities are reordered, new stop pairs trigger fresh direction lookups (dependency includes stop names). Failed fetches show "N/A" via `_fetched` marker instead of infinite "Loading...".

**Meal restaurant suggestions:** Each meal slot (Breakfast/Lunch/Dinner) has a "Find places to eat nearby" button. Clicking it fetches 5 restaurants from `/api/restaurants` using the nearest activity's GPS coordinates (or hotel for breakfast, or city geocoding fallback for arrival days). Restaurants shown as horizontal scrollable cards with photo, name, rating, cuisine. Selecting one adds it as a **custom activity** at the meal time (appears as a full activity card with photo, category pill, directions). Selected venue stored in `deepPlanData.selectedMealVenues` (key format: `day_2_lunch`). Meal venues get forced `restaurant` category for card styling. Duration: breakfast 30min, lunch 45min, dinner 60min. Custom activities inserted at correct chronological position (not always before dinner).
...
**Persisted to DB:** Custom activities, day notes, day start times, AI city activities cache, day themes, activity order, edited times, removed activities, selected meal venues, and `cacheGeneratedAt` stored in `deepPlanData` (via `_deepPlanData` JSONB embedding).
...
- **Activity GPS coordinates:** `ItineraryActivity`, `CityActivityCached`, and `DeepStop` all have optional `lat`/`lng`. AI prompt requests GPS coordinates for every activity. Used for precise Google Directions queries (falls back to `"name, city"` when missing). Bump `FIX_DATE` when changing AI output format to force regeneration of cached activities.
...
- **AI provider chain:** `itinerary-activities` uses Gemini 3.1 Flash Lite (primary) → OpenAI Chat Completions (fallback, 3 retries) → Anthropic (last resort). PDF-handling endpoints (`extract-trip`, `extract-booking`, `extract-transport`, `classify-doc`) use OpenAI Responses API (`/v1/responses`) for file/image support. Gemini response: `data.candidates[0].content.parts[0].text`. OpenAI Chat: `data.choices[0].message.content`. OpenAI Responses: `data.output[].content[].text`.
- **Same-airport detection:** Route page auto-select detects when origin and destination resolve to the same IATA code (e.g., Thane→Mumbai both resolve to BOM) and auto-selects drive/cab. Works globally via the flights API's `fromResolved`/`toResolved` codes — no hardcoded city lists.
- **PDF export:** Rich day-by-day itinerary with activity times/categories/prices, dual currency display (selected + INR), full budget breakdown (flights, trains, hotels, attractions, food, local transport). Hotel costs correctly multiply by room count. Flight costs use `adults + children + infants*0.15`.
...
GEMINI_API_KEY                # AI itinerary activities (primary — Gemini 3.1 Flash Lite)
