...
- `/api/ai/itinerary-activities` — **Chat Completions API** (`/v1/chat/completions`) with `gpt-4.1-mini`. Generates city activities with durations, categories, opening hours, ticket prices, and **GPS coordinates** (`lat`/`lng`). Per-provider 45s timeout via AbortController. Multi-day aware: assigns `dayIndex` per activity + `dayThemes` for logical day progression. No static fallback — returns empty array if AI fails; UI shows "Auto-plan" prompt. Response cached in `deepPlanData.cityActivities`. Requests 7 activities per day + 3 extras. `max_tokens: 4096`. **Critical:** Do NOT use the Responses API (`/v1/responses`) for this route — it's unreachable from Railway asia-southeast1 (all requests timeout). **All AI routes require authentication** (`getServerSession`).
...
- **OpenAI API split:** PDF-handling endpoints (`extract-trip`, `extract-booking`, `extract-transport`, `classify-doc`) use Responses API (`/v1/responses`) for file/image support. `itinerary-activities` uses Chat Completions API (`/v1/chat/completions`) — the Responses API is unreachable from Railway asia-southeast1. Content types for Responses: `input_text`, `input_file`, `input_image`. Response: `data.output[].content[].text`. Chat Completions: `data.choices[0].message.content`.
...
GEMINI_API_KEY                # AI itinerary activities (Gemini fallback when OpenAI times out)
